...
export function taylorSeries(equation:string,point:string,order:string){ //Takes an equation, the point where the equation is centered, and the order
  ...
  //Put LaTeX into my HTML Document
  //Put the taylor series equation on the html page, then steps
  // Steps should be an 2d array of each c1, c2, c3 and coefficient, then 
    ...
    while (count <= order1-1){
      ...
      let c3 = nerdamer('x - ' + point1).pow(i).evaluate().text('decimals'); //Power
      ...
      return resultLocal; //Reworked lets gooooo
  }
